...
describe('ShareYoutubeVideoDialog Component', () => {
  it('should render normal', () => {
    ...
    expect(
      screen.getByLabelText('toggle share youtube video'),
    ).toBeInTheDocument();
  });
  ...
  it('should share a YouTube video and trigger success toast', async () => {
    ...
    require('@/services/video').shareYoutubeVideo.mockResolvedValue(
      mockShareResponse,
    );
    ...
    await waitFor(() => {
      expect(
        require('@/services/video').shareYoutubeVideo,
      ).toHaveBeenCalledTimes(1);
      expect(toast.success).toHaveBeenCalledTimes(1);
      expect((toast.success as jest.Mock).mock.calls[0][0]).toEqual(
        'Share video successful.',
      );
    });
  });
  ...
  it('should show error message when youtube video Id invalid', async () => {
    ...
    await waitFor(() => {
      ...
      expect((toast.error as jest.Mock).mock.calls[0][0]).toEqual(
        'Youtube ID is not correct! Please check again.',
      );
    });
  });
});
