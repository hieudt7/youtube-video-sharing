...
export const worker = setupWorker(
  ...
  rest.get('/api/get-all-video', (_req, res, ctx) => {
  ...
  rest.get('/api/get-video-action', (_req, res, ctx) => {
    const storedVideoAction = JSON.parse(localStorage.getItem('videoAction')!);
    const user = JSON.parse(localStorage.getItem('userLogged')!);
    if (!user) {
      return res(
        ctx.status(401),
        ctx.json({ success: false, message: 'not authorized' }),
      );
    } else {
  ...
  rest.post<UserInfo>('/api/login', (req, res, ctx) => {
    ...
    if (userLogged && password === userLogged.password) {
      ...
      return res(
        ctx.status(401),
        ctx.json({ success: false, message: 'Login failed' }),
      );
    }
  }),
  //mock api register
  rest.post<UserInfo>('/api/register', (req, res, ctx) => {
    ...
    if (existingUser) {
      return res(
        ctx.status(401),
        ctx.json({ success: false, message: 'User already exists.' }),
      );
    } else {
      //mock user register
      const newUser = {
